...
* ivy - Awesome interface for nearly everything
  ...
  Fuzzy search in current buffer, selection in the minibuffer
  #+BEGIN_SRC emacs-lisp
    (use-package swiper
      :ensure t
      :config
  ...
  =smex= ranks the importance of selections after M-x
  #+BEGIN_SRC emacs-lisp
    (use-package smex
      :ensure t
      :config
  ...
  Show more information in the minibuffer when using =ivy=
  #+BEGIN_SRC emacs-lisp
...
* Python
  =jedi= for navigation and auto completion
  #+BEGIN_SRC emacs-lisp
...
* Org
  ...
  Make black color themes work nicely when exported to html
  #+BEGIN_SRC emacs-lisp
